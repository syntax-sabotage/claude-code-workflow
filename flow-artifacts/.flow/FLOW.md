...
| Command | Purpose |
|---------|---------|
| `/flow-status` | Query milestones + issues via `gh` |
| `/flow-start <issue#>` | Claim issue + create branch |
| `/flow-stream <name>` | Query/create milestone, list stream issues |
| `/flow-objective <title>` | `gh issue create` with milestone + labels |
| `/flow-ship` | Close issue + PR + merge + deploy |
| `/flow-triage` | Assign milestones to `needs-triage` issues |
| `/flow-reflect <stream>` | Synthesize learnings -> `.context/`, add `reflected` label |
